                ...
                <div class="col-lg-6 mb-5 mb-lg-0 position-relative">
                    ...
                    <div class="card bg-glass">
                        <div class="card-body px-4 py-5 px-md-5">
                            <form method="get" action="{{ route('testDelete') }}" enctype="multipart/form-data">
                                @csrf
                                <div class="mb-3">
                                    <select class="form-select" name="testId" required>
                                        <option selected disabled value="">Choose the variant</option>
                                        @foreach ($tests as $test)
                                            <option value="{{ $test->id }}">{{ $test->nameImage }}</option>
                                        @endforeach
                                    </select>
                                </div>
